//
//  TodoListInteractor.h
//  CleanArchitectureForObjective-C
//
//  Created by Naoki_Sawada on 2017/02/21.
//  Copyright (c) 2017年 CAFObjC. All rights reserved.
//
//  This file was generated by the Clean ObjectiveC Xcode Templates so you can apply
//  clean architecture to your iOS and Mac projects, base on http://clean-swift.com

@import Foundation;

@class TodoListWorker;
@class TodoListRequest;
@class TodoListResponse;

@protocol TodoListInteractorOutput <NSObject>
@required
- (void)presentSomething:(TodoListResponse *)response;
@end

@protocol TodoListInteractorInput <NSObject>
@required
- (void)fetchTodoItems:(TodoListRequest *)request;
@end

@interface TodoListInteractor : NSObject <TodoListInteractorInput>
@property (nonatomic) id<TodoListInteractorOutput> output;
@property (nonatomic) TodoListWorker *worker;
@end
